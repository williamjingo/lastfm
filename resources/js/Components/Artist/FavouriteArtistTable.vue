        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tr v-for="artist in artists.data" :key="artist.id" class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="py-4">
                    <button
                        type="button"
                        @click="$emit('artist-click', {action: 'remove-artist', data: artist})"
                        class="text-white bg-purple-700 hover:bg-indigo-800 focus:outline-none focus:ring-4 focus:ring-indigo-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mb-2 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-900">Remove Favourite</button>
                </td>
            </tr>
            </tbody>
        </table>
